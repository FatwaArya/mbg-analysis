...
st.set_page_config(page_title="Analysis · MBG", page_icon="🔬", layout="wide")
require_auth()
...
try:
    ...
    c6.metric("Neg Amplification", "Significant ✅" if s['negative_amplification_significant'] else "Not sig")
...
st.caption("Political/corruption topics get 3–5× more retweets than average. Food poisoning spreads widely too.")
...
try:
    ...
    with col6:
        st.dataframe(shift[["label","early_neg_pct","recent_neg_pct","change_pp"]].rename(columns={
            "label":"Topic","early_neg_pct":"Early %","recent_neg_pct":"Recent %","change_pp":"Δ pp"
...
st.caption("Corruption topic (Topic 1) surged from 815 → 2,091 tweets/month in Feb–Mar 2026 — driving the negativity spike.")
...
hyps = [
    ...
    ("✅ H4", "Negativity surge traceable to specific growing topics",
     "Topic 1 (corruption) grew 815 → 2,091 tweets/month Feb–Mar 2026, driving the 52%+ negativity."),
