...
def run_batch(texts, model, label_map):
    labels, scores = [], []
...
if len(id_df) > 0:
    labels, scores = run_batch(id_df["text_clean_light"].fillna("").tolist(), sentiment_id, ID_MAP)
    id_df["sentiment_label"] = labels
    id_df["sentiment_score"] = scores
    id_df["sentiment_normalized"] = labels

if len(en_df) > 0:
    labels, scores = run_batch(en_df["text_clean_light"].fillna("").tolist(), sentiment_en, EN_MAP)
